...
class NeuralNetwork {
  ...
  generateDiscriminatorError(inputsArray, targetArray){
    let outputs = Matrix.toMatrix(inputsArray)
    let listResult = []
    listResult.push(outputs)
    this.layers.forEach( (layer, index) => {
    ...
    let whoT = Matrix.transpose(this.layers[0]);
    let err = Matrix.multiply(whoT, errors)
    errors = err

    return Matrix.toArray(errors)
  }
...
// ADD BIAS TO DRAW FUNCTION
  static draw(nn, context, inputs){
    ...
    inputs.forEach( (elem, index) => { // Voir si on peut draw les inputs dans la boucle du dessous ou s'il faut vraiment séparer
...
let listInputs
let discriminator = new NeuralNetwork([784, 1])
discriminator.setActivationFunction(sigmoid)
let generator = new NeuralNetwork([10, 784])
generator.setActivationFunction(sigmoid)
generator.setLearningRate(0.05)

let seedGenerator = []
for(let i = 0; i < 10; i++) seedGenerator[i] = Math.random()
const training = () => {
  if(Math.random() > 0.99){ // Training discriminator on real data
    ...
    discriminator.train(inputs, [1])
  }

  // ECRIRE D(G(Z)) directement

  else{ // Training discriminator and generator with generator value
    let outputGenerator = generator.think(seedGenerator)
    // discriminator.train(outputGenerator, [0])

    let outputDiscriminator = discriminator.think(outputGenerator)

    // console.log(outputDiscriminator)
    let errorGenerator = 1 - outputDiscriminator
    console.log(errorGenerator)
    // let errorG = []
    // for( let i = 0; i < 784; i++) errorG[i] = errorGenerator
    // // generator.train([seedGenerator], [errorGenerator])
    console.log("oui")

    let errorG = discriminator.generateDiscriminatorError(outputGenerator, [0])


    generator.train(seedGenerator, errorG)
    console.log("non")
