...
let listInputs
let discriminator = new NeuralNetwork([784, 56, 28, 14, 7, 1])
discriminator.setActivationFunction(sigmoid)
discriminator.setLearningRate(0.01)

const training = () => {
  if(Math.random() > 0.5){ // Training discriminator on real data
    ...
    // console.log(discriminator.think(inputs)[0])
  }


  else{ // Training discriminatoron fake data
    let inputs = []
    for( let i = 0; i < 784; i++){
      inputs[i] = Math.random()
      if(Math.random() < 0.25) inputs[i] = 0
      else if(Math.random() < 0.5) inputs[i] = 0.25
      else if(Math.random() < 0.75) inputs[i] = 0.5
      else inputs[i] = 1
    }

    // console.log(discriminator.think(inputs)[0])
    discriminator.train(inputs, [0])
...
const guess = () => {
  ...
  let results = discriminator.think(inputs)
